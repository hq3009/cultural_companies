...
DROP TABLE IF EXISTS wenhua_and_lvyou_company;

-- 创建文化兼旅游行业企业子表
CREATE TABLE wenhua_and_lvyou_company AS
SELECT * FROM company_extended
WHERE INDV_NM IN (
    -- 文化兼旅游行业代码列表
...
SELECT
    ...
    COUNT(*) AS total_company, -- 文化旅游企业总数
...
SELECT
    ...
    SUM(CASE WHEN INCLU_REASON IS NOT NULL THEN 1 ELSE 0 END) AS abnormal_operation_company, -- 经营异常企业数
    SUM(CASE WHEN SERILL_REA IS NOT NULL THEN 1 ELSE 0 END) AS serious_violation_company, -- 严重违法失信企业数
    SUM(CASE WHEN EXECUTED_COURT IS NOT NULL THEN 1 ELSE 0 END) AS dishonest_executed_company, -- 失信被执行人企业数
    SUM(CASE WHEN CONGNIZ_ORG IS NOT NULL THEN 1 ELSE 0 END) AS tech_small_company, -- 科技型小微企业数
    SUM(CASE WHEN TAX_CREDI_LEVEL = 'A' THEN 1 ELSE 0 END) AS grade_a_taxpayer_company, -- A级纳税人企业数
    SUM(CASE WHEN WEB_STORE_NM IS NOT NULL THEN 1 ELSE 0 END) AS company_with_website, -- 有网站网店企业数
    SUM(CASE WHEN ADMIN_PUNISH_DOC IS NOT NULL THEN 1 ELSE 0 END) AS company_with_penalty, -- 有行政处罚企业数
    SUM(CASE WHEN ALT_DT IS NOT NULL THEN 1 ELSE 0 END) AS company_with_equity_change, -- 有股权变更企业数
    SUM(CASE WHEN CREDITOR IS NOT NULL THEN 1 ELSE 0 END) AS company_with_guarantee -- 有对外担保企业数
FROM wenhua_and_lvyou_company;
...
SELECT
    ...
    COUNT(CASE WHEN sb.PENS_NUM IS NOT NULL THEN 1 END) AS company_with_social_insurance, -- 参与社保企业数
    COUNT(CASE WHEN sb.PENS_NUM IS NULL THEN 1 END) AS company_without_social_insurance, -- 未参与社保企业数
...
FROM wenhua_and_lvyou_company e
